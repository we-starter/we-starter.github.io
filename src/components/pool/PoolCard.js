...
import { useLeftTime, usePoolCard } from './Hooks';
        ...
        <Grow in={show} timeout={1500}>
            <div>
                <div className='card-pool__item'>
                    ...
                    <Link to={pool.link} className='card-pool__stake'>
                        <FormattedMessage id='poolText11' />
                        <b>{pool.pair}</b>
                    ...
                    <div>
                        <dl className='card-pool__dl'>
                            ...
                            <div className='card-pool__dl-row'>
                                <dt className='card-pool__dl-dt'>
                                    <FormattedMessage id='poolText14' />
                                </dt>
                                <dd className='card-pool__dl-dd'>
                                    <b className='card-pool__dl-blue'>
                                        $
                                        {total &&
                                        price &&
                                        pool.type !== 'ETH' &&
                                        pool.type !== 'USDT'
                                            ? new BigNumber(formatAmount(total))
                                                  .multipliedBy(price)
                                                  .toString()
                                            : REQUESTING_DATA}
                                    </b>
                            ...
                            <div className='card-pool__dl-row'>
                                <dt className='card-pool__dl-dt'>APY</dt>
                                <dd className='card-pool__dl-dd'>
                                    <b className='card-pool__dl-green'>
                                        {pool.totalRewards === -1
                                            ? '>5000%'
                                            : glfPrice &&
                                              totalRewards &&
                                              price &&
                                              total
                                            ? `${new BigNumber(glfPrice)
                                                  .multipliedBy(
                                                      toWei(pool.totalRewards)
                                                  )
                                                  .multipliedBy(365)
                                                  .dividedBy(14)
                                                  .dividedBy(
                            ...
                            <div className='card-pool__dl-row'>
                                <dt className='card-pool__dl-dt'>
                                    <FormattedMessage id='poolText15' />
                                </dt>
                                <dd className='card-pool__dl-dd'>
                                    <b className='card-pool__dl-black'>
                                        {leftTime
                                            ? `${leftTime.days}d ${leftTime.hours}h ${leftTime.minutes}m`
                                            : REQUESTING_DATA}
                                    </b>
                                </dd>
                            </div>
